...
Exact whitespace and linebreaks are not enforced, and the input may work well without the recommended structure. However, in order to avoid issues it is best to work by the following guidelines. The input is seperated into three blocks: the `$molecule` block, `$scf` block and `$basis` block. The order of the blocks does not matter.
...
If a basis set is not available and it exists in the Basis Set Exchange, you may download and add it to the `basis_sets` directory. It will then be available for use.
...
|**SYMMETRY_TOL**|`float`|`1e-5`|Tolerance for symmetry detection.|
...
|**GUESS_MIX**|`int`|`0`|Mix HOMO/LUMO to break symmetry (0-10). The new HOMO is given by: $\frac{1}{\sqrt{1+k^2}}\left(\mathrm{HOMO}+k\cdot\mathrm{LUMO}\right)$, and the LUMO is given by: $\frac{1}{\sqrt{1+k^2}}\left(\mathrm{LUMO}-k\cdot\mathrm{HOMO}\right)$, where $k =\frac{\mathrm{GUESS\\_MIX}}{10}$.|
...
Two electron integrals are calculate a shell-quartet at a time.
A "shell" is defiend as a group of basis functions shcaring the same exponent, center, and total angular momentum.
This structure allows many intermediate quantities to be reused regardless of the specific angular momentum distribution (e.g., $d_{xy}$ vs $d_{z^2}$).

**Note on Future Work:** I am currently working on migration to the Gill and Pople **PRISM algorithm** [[2]](#2)-[[4]](#4) on the PRISM branch.
...
If this bond is below `SCWARTZ_THRESH`, the quartet is skipped.
The $\langle ij|ij \rangle$ terms can be calculated in comparatively negligble $O(n^2)$ time.
...
This allows for more efficient density screening, especially in later iterations.

### DIIS Convergence Acceleration

Direct Inversion in the Iterative Subspace (DIIS) extrapolates the Fock matrix from previous iterations to minimize and error residual [[8]](#8).
...
The program detects the molecular point group by classifying atoms into sets of Symmetrically Equivalent Atoms (SEAs) by computing the interatomic distance matrix, and taking into acount the atomic numbers, finding atoms with equivalent distance profiles.
The SEAs are used to generate a small set of possible symmetry elements, which are tested by applying the corresponding symmetry operations on the molecular geometry. If the transformed geometry is a permutation of the original geometry (up to a tolerance), the symmetry operation is valid. A decision tree algorithm utilizes the information from the SEAs and prinicpal moments of inertia to determine the point group and principal axes.[[11]](#11)
...
*Note: Currently, symmetry is used for point group detection and geometry oreintation only. Use of Symmetry adapted Linear Combinations (SALCs) for block-diagonalizing the Fock matrix and integral screening (two electron-integrals are only non-zero if the direct product of the irreducible representations of the four orbitals contains the totally symmetric irreducible representation of the molecule's point group) are planned for future releases.*
